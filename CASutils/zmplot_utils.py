...
def plotlatlogpre_100to0p1(fig, data, lat, pre, ci, cmin, cmax, titlestr, x1=0.1, x2=0.9, y1=0.1, y2=0.9, ylabel=True, yticklabels=True, fsize=11, cmap='blue2red', xlim=None):
    """
    Plot a pressure versus latitude contour plot from 100hPa up to 0.1hPa.
    """
    ...
    ax.contourf(lat,-1.*np.log10(pre), data, levels=clevs, cmap=mymap, extend='max')
    ax.contour(lat,-1.*np.log10(pre), data, levels=clevs[ clevs != 0], colors='black', linewidths=0.5)
    ax.set_ylim(-np.log10(100.),-np.log10(0.1))
    ax.set_yticks([-np.log10(100),-np.log10(10),-np.log10(1),-np.log10(0.1)])
...
def plotlatlogpre_to0p01(fig, data, lat, pre, ci, cmin, cmax, titlestr, x1=0.1, x2=0.9, y1=0.1, y2=0.9, ylabel=True, yticklabels=True, fsize=11, cmap='blue2red'):
    """
    Plot a pressure versus latitude contour plot up to 0.01hPa.
    """
